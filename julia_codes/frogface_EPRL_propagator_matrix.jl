...
printstyled("\nFrogface EPRL propagator matrix parallelized on $(number_of_workers) worker(s) and $(number_of_threads) thread(s)\n\n"; bold=true, color=:blue)
...
function frogface_EPRL_propagator_matrix(cutoff, shells, jb)
    ...
    for pcutoff = 0:step:cutoff
        ...
        @sync @distributed for bulk_spins in bulk_spins_pcutoff
            ...
            v1 = vertex_compute([jb, jb, jb, jb, j34, j25, j25, jb, jb, j45], shells; result=result_return)
            v2 = vertex_compute([jb, jb, jb, jb, j23, jb, j25, jb, j25, j34], shells; result=result_return)
            ...
            for column_index = 1:1:DIM, row_index = column_index:1:DIM
                ...
                @inbounds for i2 in 1:i2_range, i3 in 1:i3_range, i4 in 1:i4_range, i5 in 1:i5_range
                    amp += v1.a[i5, i5, i3, i4, row_index] * v2.a[i4, i3, i2, i2, column_index]
                end
...
@time frogface_EPRL_propagator_matrix(1, 0, half(1));
println("done\n")
sleep(1)
...
for Dl = SHELL_MIN:SHELL_MAX
    ...
    @time propagator_matrix = frogface_EPRL_propagator_matrix(CUTOFF, Dl, BOUNDARY_SPIN)
    ...
    for pcutoff = 0:0.5:CUTOFF_FLOAT
        ...
        CSV.write("$(FINAL_SHELL_PATH)/frogface_pcutoff_$(pcutoff).csv", df)
    end
