...
printstyled("\nGoat 6 faces BF divergence parallelized on $(nworkers()) worker(s)\n\n"; bold=true, color=:blue)
...
function goat_BF(cutoff)
    ...
    for pcutoff = 0:step:cutoff
        ...
        bulk_spins_pcutoff = NTuple{6,HalfInt}[]

        for j25::HalfInt = 0:onehalf:pcutoff, j45::HalfInt = 0:onehalf:pcutoff,
            j23::HalfInt = 0:onehalf:pcutoff, j33::HalfInt = 0:onehalf:pcutoff,
            j44::HalfInt = 0:onehalf:pcutoff, j34::HalfInt = 0:onehalf:pcutoff


            # skip if computed in lower partial cutoff
            j25 <= (pcutoff - step) && j34 <= (pcutoff - step) &&
                j23 <= (pcutoff - step) && j45 <= (pcutoff - step) &&
                j44 <= (pcutoff - step) && j33 <= (pcutoff - step) && continue

            # skip if any intertwiner range empty
            i1, _ = intertwiner_range(jb, j25, j25, jb)
            i2, _ = intertwiner_range(j34, jb, j25, jb)
            i3, _ = intertwiner_range(jb, j33, j45, j23)
            i4, _ = intertwiner_range(j23, j25, j45, j44)
            ...
            # must be computed
            push!(bulk_spins_pcutoff, (j25, j45, j23, j33, j44, j34))
        ...
        @time tampl = @sync @distributed (+) for bulk_spins in bulk_spins_pcutoff
            ...
            i3, i3_range = intertwiner_range(jb, j33, j45, j23)
            i4, i4_range = intertwiner_range(j23, j25, j45, j44)

            # compute BF vertices
            v1 = vertex_BF_compute([jb, j25, j25, jb, jb, jb, jb, j34, jb, jb])
            v2 = vertex_BF_compute([jb, j25, j25, jb, j23, j45, j33, j44, j45, j23])

            # dim internal faces
            dfj = dim(j25) * dim(j34) * dim(j45) * dim(j23) * dim(j44) * dim(j33)

            # intertwiner contractions
            amp = 0.0

            for i1 in 1:i1_range, i3 in 1:i3_range, i2 in 1:i2_range, i4 in 1:i4_range
                amp += v1.a[ib_index, i2, i2, ib_index, i1] * v2.a[i3, i4, i4, i3, i1]
            end
...
CSV.write("$(STORE_FOLDER)/goat_6_faces.csv", df)
